		...
		if not paused then
			if not doingDialogue then
				...
				for i = 1, 4 do
					...
					if not settings.botPlay then
						if input:down(curInput) or paddy.dpad.isDown(curInput) then
							holdingInput = true
						else
							holdingInput = false
						end

						if input:pressed(curInput) or paddy.dpad.isDown(curInput) then
							--local success = false
					...
					if not settings.botPlay then
						if notMissed[noteNum] and #boyfriendNote > 0 and (input:down(curInput) or paddy.dpad.isDown(curInput)) and ((not settings.downscroll and boyfriendNote[1].y - musicPos <= -400) or (settings.downscroll and boyfriendNote[1].y - musicPos >= 400)) and (boyfriendNote[1]:getAnimName() == "hold" or boyfriendNote[1]:getAnimName() == "end") then
							voices:setVolume(1)
						...
						if input:released(curInput) or not paddy.dpad.isDown(curInput) then
							boyfriendArrow:animate("off", false)
						end
					else
